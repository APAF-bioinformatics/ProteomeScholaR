...
## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
base_dir <- here::here()
data_dir <- file.path( base_dir, "Data", "PLoS_Biology_2019")
source_dir <- file.path(base_dir, "..", "Shared")
fasta_output_dir <- file.path(base_dir, "Results", "PLoS_Biology_2019", "Phosphopeptides")


## ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
results_dir <- file.path(base_dir, "Results", "PLoS_Biology_2019", "Phosphopeptides") 
fasta_file <- file.path( data_dir,   "16May2017uniprot-RattusIso.fasta")
fasta_meta_file <-  file.path( fasta_output_dir, "aa_seq_tbl.RDS") 
raw_counts_file <- file.path( data_dir, "evidence.txt") 
site_prob_threshold <- 0.75 # phosphosite probability threshold
recover_site_prob_thresh <- 0.5
col_pattern_string <- "Reporter intensity"
add_cols_string <- ""
...
if( length(command_line_options ) > 0 ) {
  ...
  if( add_cols_string == "" ) { warnings("No value for --add-columns was supplied.") }
