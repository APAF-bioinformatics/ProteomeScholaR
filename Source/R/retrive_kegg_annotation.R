...
# pathways_download <- downloadPathways(species, out.dir= file.path( data_dir, "KEGG")) ## only use this line if you want to download all the XML files locally
pathways <- downloadPathways(species)
# pathways$mmu05416

#step 4: retrieve gene sets for an organism from databases such as GO and KEGG:
gene_sets <- getGenesets(org = species, db = "kegg", cache = TRUE, return.type="list")
