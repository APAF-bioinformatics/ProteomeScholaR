...
```{r}
  evidence_tbl_del_obsolete <- evidence_tbl_cleaned %>%
    mutate( uniprot_acc = str_split( leading_proteins, ";") ) %>%
    unnest( uniprot_acc )   %>%
    mutate( cleaned_acc = cleanIsoformNumber(uniprot_acc)) %>%
    left_join( aa_seq_tbl,
               by=c("uniprot_acc" = "uniprot_acc",
                    "cleaned_acc" = "cleaned_acc") ) %>%
    mutate( leading_proteins =  case_when(is.na(seq) ~ proteins,
                                          TRUE ~ leading_proteins)  ) %>%
    dplyr::select(one_of(colnames( evidence_tbl_cleaned )))

  resolve_acc_helper <- evidence_tbl_del_obsolete %>%
    dplyr::select( evidence_id, proteins, leading_proteins, cleaned_peptide) %>%
    mutate( uniprot_acc = str_split( leading_proteins, ";") ) %>%
    unnest( uniprot_acc )   %>%
    mutate( cleaned_acc = cleanIsoformNumber(uniprot_acc)) %>%
    left_join( aa_seq_tbl,
    ...
    dplyr::select(evidence_id, one_of(c( "uniprot_acc", "gene_name", "cleaned_acc", "proteins",
                                          "protein_evidence", "status", "is_isoform", "isoform_num", "seq_length"  ))) %>%
    distinct %>%
    arrange( evidence_id, protein_evidence, status, is_isoform, desc(seq_length), isoform_num )

  print( colnames(head(resolve_acc_helper)) )

  resolve_acc_helper %>%
dplyr::filter(  str_detect(uniprot_acc, "F1LMQ9") | str_detect(uniprot_acc, "B0BN49"))
  

  score_isoforms <- resolve_acc_helper %>%
    mutate( gene_name = ifelse( is.na(gene_name) | gene_name == "", "NA", gene_name)) %>%
    group_by( evidence_id,  gene_name ) %>%
    arrange( evidence_id,  protein_evidence,
  ...
  group_gene_names_and_uniprot_accs <- score_isoforms %>%
    distinct( evidence_id, gene_name, ranking ) %>%
    dplyr::filter( ranking == 1) %>%
    left_join( score_isoforms %>%
                 dplyr::select( evidence_id, ranking, gene_name, uniprot_acc),
               by = join_names )   %>%
...
```{r}
accession_gene_name_tbl %>%
  dplyr::filter( uniprot_acc  == "B5DEL4" )
