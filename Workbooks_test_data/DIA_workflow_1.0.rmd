...
c(
  ...
  time_dir <- file.path(qc_dir, timestamp), ##this is a way to distinguish between different QC runs of the same data in case you change params
...
```{r Sample cleanup and design matrix construction}
#' # Flexible string extraction from underscore-separated strings
#' # Example string: "20140602_ffs_expt1_r1_junk"
...
data_cln <- data_tbl |>
        #Remove any unwanted information from your sample names
        mutate(Run = extract_experiment(Run, mode = "start")) |>
        # Ensure that the Precursor.Normalised and Precursor.Quantity columns are numbers
        mutate(Precursor.Normalised = as.numeric(Precursor.Normalised)) |>
        mutate(Precursor.Quantity = as.numeric(Precursor.Quantity)) 

# Create an empty design matrix with Run IDs and placeholder columns for metadata
if (!exists("design_matrix_raw")) { #NEED TO UPDATE THIS WITH WHETHER THE FILE LOCATION EXISTS, AND IF NOT, CREATE IT
  design_matrix_raw <- tibble(
    Run = data_cln |> 
      # Get unique Run values, removing duplicates
      distinct(Run) |>
      # Keep only the Run column
      dplyr::select(Run) |>
      # Extract only the numeric portion from Run IDs (e.g., "51655" from "RPMI_51655")
      dplyr::mutate(Run = stringr::str_extract(Run, "\\d+")) |>
      # Convert from data frame to vector
      dplyr::pull(Run),
    # Add empty character column for group assignments
    group = NA_character_,
    # Add empty integer column for replicate numbers
...
## This section initializes a PeptideQuantitativeData object with peptide-level quantitative data and experimental design information.
## It specifies the column names for various data attributes and sets up the design matrix for the experiment.
## This makes it easier to manage the data and perform quality control checks and normalization steps.
## For example, the object can be used to filter out peptides with low intensity values or missing values, or to normalize the data between samples.
## In the next chunk, the object will be used to perform quality control checks and normalization steps.
...
peptide_normalized_pif_cln@peptide_data |>
    group_by(Run) |>
    summarise(n_proteins = n_distinct(Protein.Ids), .groups = "drop") |>
    arrange(Run)  # Sort by Run ID
...
removed_peptides_with_less_than_two_peptides
```
